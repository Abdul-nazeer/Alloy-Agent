...
def report_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        # Fallback: create template-based report
        report_lines = [
            f"# Maintenance Report: {equipment_id}",
            f"**Date:** {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}",
            f"**Equipment Type:** {equipment_type}",
            f"**Risk Level:** {risk_level}",
            "",
            "## Executive Summary",
            f"{equipment_type} {equipment_id} analysis completed.",
        ]
        ...
        if state.get("root_causes"):
            ...
            for i, rc in enumerate(state["root_causes"], 1):
                report_lines.append(f"{i}. **{rc.cause}** (Confidence: {rc.confidence*100:.0f}%)")
        ...
        if state.get("anomalies_detected"):
            ...
            for anomaly in state["anomalies_detected"]:
                report_lines.append(f"- **{anomaly.severity}**: {anomaly.message}")
        ...
        if state.get("recommendations"):
            ...
            for i, rec in enumerate(state["recommendations"], 1):
                report_lines.append(f"{i}. **Priority {rec.priority}**: {rec.action}")
                if rec.estimated_time:
                    report_lines.append(f"   - Estimated time: {rec.estimated_time}")
